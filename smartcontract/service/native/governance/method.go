...
func withdrawTotalStake(native *native.NativeService, contract common.Address, address common.Address, stake uint64) error {
	...
	if totalStake.Stake < stake {
		return fmt.Errorf("withdraw, tst deposit is not enough")
	}
...
func executeSplit(native *native.NativeService, contract common.Address, view uint32) error {
	...
	balance, err := getTsgBalance(native, utils.GovernanceContractAddress)
	if err != nil {
		return fmt.Errorf("executeSplit, getTsgBalance error: %v", err)
	}
	//get globalParam
	globalParam, err := getGlobalParam(native, contract)
	if err != nil {
	...
	for i := 0; i < int(config.K); i++ {
		...
		err = appCallTransferTsg(native, utils.GovernanceContractAddress, address, nodeAmount)
		if err != nil {
			return fmt.Errorf("executeSplit, tsg transfer error: %v", err)
		}
	}
...
func executeSplit2(native *native.NativeService, contract common.Address, view uint32) (uint64, error) {
	...
	balance, err := getTsgBalance(native, utils.GovernanceContractAddress)
	if err != nil {
		return splitSum, fmt.Errorf("executeSplit, getTsgBalance error: %v", err)
	}
	...
	if gasAddress.Address == common.ADDRESS_EMPTY {
		...
		err := appCallTransferTsg(native, utils.GovernanceContractAddress, gasAddress.Address, dappIncome.Uint64())
		if err != nil {
			return splitSum, fmt.Errorf("appCallTransferTsg, appCallTransferTsg error: %v", err)
		}
	}
