...
* [Tesranode CLI Instruction](#tesranode-cli-instruction)
	* [1. Start and Manage Tesranode Nodes](#1-start-and-manage-tesranode-nodes)
		...
		* [1.2 Node Deployment](#12-node-deployment)
			...
			* [1.2.3 Deploying on public test network Scorpio sync node](#123-deploying-on-public-test-network-scorpio-sync-node)
...
--config
The config parameter specifies the file path of the genesis block for the current Ontolgy node. If not specified, Tesranode will use the config of Scorpio TestNet. Note that the genesis block configuration must be the same for all nodes in the same network, otherwise it will not be able to synchronize blocks or start nodes due to block data incompatibility.
...
--networkid
The networkid parameter is used to specify the network ID. Different networkids cannot connect to the blockchain network. 1=main net, 2=scorpio test net, 3=testmode, and other for custom network.
...
#### 1.2.3 Deploying on public test network Scorpio sync node
